...
- 逻辑：读取 `output/<date>/date_<date>.xlsx` → 特征提取（公告类型 / 需求品类 10 类 / 采购人主体 6 类 / 金额「元|万元」两单位 / **关键时间节点：投标截止·开标·获取文件截止**）→ 生成 5 张图表（品类 / 地区 / 类型 / 采购人 / 金额规模）到 `output/<date>/charts/<date>_*.png` → 输出 Markdown 报告 `output/<date>/需求洞察报告_<date>.md`。
- 报告含章节：Abstract / 1.Introduction / 2.需求品类结构 / 3.区域需求版图 / 4.金额规模与重点机会（含 **4.4 投标截止时间节奏：紧迫度分布 + 即将到期 TOP10**）/ 5.Conclusion / 6.References。
...
- LLM 精读：`.env` 中 `LLM_ENABLED=true` 后，经 **deerflow cli**（本机 Gateway）执行，自动挑选精读条目（**直接相关全部 + 其余按「有金额优先、金额降序」补足，清单带 [编号] 供模型引用防串位**，items 格式 `- [编号][地区][相关度] 标题` + 金额/截止/开标/正文摘要/链接），模型可 `web_fetch` 打开链接逐条核实详情，产出报告写入沙箱 `/mnt/user-data/outputs/商机分析_{date_key}.md` 并 present_files 交付，deerflowcli 下载到本地 `output/<date>/deerflow/` 后读取作为精读结果；`LLM_LIMIT` 仅作兜底上限（默认 15，设 0 不限），正常无需手动改；执行失败自动降级为纯规则引擎，不影响主流程。**长任务保护（方案A+B）**：提交 deerflow 任务带 `--recursion-limit`（默认 300，`DEERFLOW_RECURSION_LIMIT` 可调，服务端 clamp 上限 1000，避免 30+ 条逐条核实触发 LangGraph `GRAPH_RECURSION_LIMIT`）；首次执行未产出报告文件时，自动从 deerflowcli 输出解析会话 ID，用 `--thread-id <id> "继续"` 恢复同一会话（让模型基于沙箱已写文件补齐并交付），而非新建会话重头跑。**精读输出自动校验（方案1+3）**：解析模型文本中的 `[编号]` 引用，引用清单范围（1~len）外的越界编号给出 ⚠️ 警告（防幻觉）；同时把被引用的合法编号映射回清单真实项目（地区/标题/真实金额/真实投标截止/相关度）生成**对照表**附在 LLM 洞察章节后，供人工核对模型说法是否属实；无引用或空清单自动跳过。每日分析与窗口期查询均生效。

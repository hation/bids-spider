...
def build_report(df, date_key, charts):
    ...
    if n_tl:
        ...
        bins_tl = [(-10**9, -1), (0, 7), (8, 14), (15, 30), (31, 10**9)]
        labels_tl = ['已过期', '7天内（立刻处理）', '8-14天（抓紧准备）', '15-30天', '30天以上（从容跟进）']
        cnt_tl = []
...
时间节奏提示了市场操作纪律：**7 天内截止的项目必须立刻投入资源**（标书、保证金、盖章不能拖），8-14 天是抓紧准备窗口，14 天以上可从容跟进。若您的团队按周做投标计划，应将"截止时间往前推 7 天"作为内部截点，预留标书制作、盖章、保证金缴纳的缓冲。

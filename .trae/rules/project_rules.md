...
- 业务配置：`config/opportunities.json`（业务名称 / 核心关键词 / 次要关键词 / 排除关键词 / 关注地区 / 金额区间 / **LLM提示词**）。**LLM 精读运行参数统一放 `.env`**：`LLM_ENABLED`（true/false）/ `LLM_API_BASE` / `LLM_MODEL` / `LLM_LIMIT`（兜底上限）/ `LLM_DETAIL_LEN`（正文摘要字数，默认400，设0不带正文）/ `LLM_API_KEY`（密钥，不入库，已 gitignore）；**提示词模板放 `config/opportunities.json` 的 `LLM提示词`**（system + user 两段，占位符 `{date_key}` / `{items}`，调整后重跑即生效）。
...
- LLM 精读：`.env` 中 `LLM_ENABLED=true` 且配置 `LLM_API_KEY` 后，自动挑选精读条目（**直接相关全部 + 其余按「有金额优先、金额降序」补足，清单带 [编号] 供模型引用防串位**），调用大模型精读（火山方舟 deepseek-v4-flash，超时 600s）；`LLM_LIMIT` 仅作兜底上限（默认 30，设 0 不限），正常无需手动改；调用失败自动降级为纯规则引擎，不影响主流程。**精读输出自动校验（方案1+3）**：解析模型文本中的 `[编号]` 引用，引用清单范围（1~len）外的越界编号给出 ⚠️ 警告（防幻觉）；同时把被引用的合法编号映射回清单真实项目（地区/标题/真实金额/真实投标截止/相关度）生成**对照表**附在 LLM 洞察章节后，供人工核对模型说法是否属实；无引用或空清单自动跳过。每日分析与窗口期查询均生效。
